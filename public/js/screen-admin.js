...
const ScreenAdmin = {
                  ...
                  return e('div', { key: ym, style: { borderRadius: 12, border: '1px solid var(--border-soft)', overflow: 'hidden' } },
                    ...
                    mOpen ? e('div', { style: { background: 'var(--bg-2)', borderTop: '1px solid var(--border-soft)' } },
                      ...clientIds.map((clientId, ci) => {
                        ...
                        const apptRate = r => { if (r.sub && cl.subclients) { const sc = cl.subclients.find(s => s.id === r.sub || s.name === r.sub); if (sc && sc.rate != null) return sc.rate; } return cl.rate || 0; };
  ...
  _admAppointments(d, s) {
    ...
    const cols = [
      ...
      { label: 'Client rate', align: 'right', render: r => {
        ...
        const displayAmt = (sub ? sub.rate : 0) || (cl ? cl.rate : 0) || 0;
  ...
  _admContractTemplates(d, s) {
    ...
    const CLIENT_SLUGS = [
      ...
      { slug: 'client-pilot', name: 'Pilot', vars: ['party', 'contact', 'email', 'vat', 'address', 'rate', 'rateStr', 'duration', 'paymentTerm', 'termDays', 'notes', 'setupFee', 'ctypeLabel', 'duurText', 'aiScopeAddition', 'aiDurationNote', 'aiSpecialConditions'] },
