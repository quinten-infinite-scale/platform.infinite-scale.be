  ...
  _adminModals(R, d, s, wrap) {
    ...
    if (k === 'clientProfile') {
      ...
      return wrap(c.name, e('div', { style: { display: 'flex', flexDirection: 'column', gap: 16 } },
        ...
        [UI.Btn('Edit', () => this.setForm('editing', true), 'soft'), c.status !== 'inactive' ? UI.Btn('Deactivate', () => { this.deactivateClient(c.id); this.closeModal(); }, 'danger') : null, UI.Btn('Delete', () => this.deleteClient(c.id, c.name), 'danger'), UI.Btn('Mark invoice paid', () => { this.markPaid(c.id); this.closeModal(); }, 'primary')], '640px');
    }
