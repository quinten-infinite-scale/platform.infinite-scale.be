  ...
  _adminModals(R, d, s, wrap) {
    ...
    if (k === 'clientProfile') {
              ...
              subs.length ? e('div', { style: { display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 10 } },
                subs.map((sc, i) => e('div', { key: i, style: { display: 'flex', flexDirection: 'column', gap: 6, padding: '10px 12px', borderRadius: 9, background: 'var(--bg-2)', border: '1px solid var(--border-soft)' } },
                  e('div', { style: { display: 'flex', alignItems: 'center', gap: 8 } },
                    e('input', { type: 'text', value: sc.name || '', placeholder: 'Sub-client name', onChange: ev => { const updated = subs.map((s, j) => j === i ? { ...s, name: ev.target.value } : s); this.setForm('editSubclients', updated); }, style: { flex: 1, padding: '4px 8px', borderRadius: 7, border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text)', fontSize: 13, fontWeight: 600, outline: 'none' } }),
                  ...
                  e('div', { style: { display: 'flex', gap: 8, alignItems: 'center' } },
                    e('input', { type: 'tel', value: sc.callback_phone || '', placeholder: 'Callback phone (e.g. +32479…)', onChange: ev => { const updated = subs.map((s, j) => j === i ? { ...s, callback_phone: ev.target.value } : s); this.setForm('editSubclients', updated); }, style: { flex: 1, padding: '4px 8px', borderRadius: 7, border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text)', fontSize: 12, outline: 'none' } }),
                    e('label', { style: { display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: 'var(--text-mute)', cursor: 'pointer', whiteSpace: 'nowrap' } },
                      e('input', { type: 'checkbox', checked: sc.whatsapp_enabled !== false, onChange: ev => { const updated = subs.map((s, j) => j === i ? { ...s, whatsapp_enabled: ev.target.checked } : s); this.setForm('editSubclients', updated); }, style: { width: 13, height: 13, accentColor: 'var(--accent)' } }),
                      'WhatsApp'))
      ...
      return wrap(c.name, e('div', { style: { display: 'flex', flexDirection: 'column', gap: 16 } },
        ...
        [UI.Btn('Edit', () => this.setForm('editing', true), 'soft'), c.email ? UI.Btn('Uitnodiging sturen', () => this.resendInvite(c.id), 'soft') : null, c.status !== 'inactive' ? UI.Btn('Deactivate', () => { this.deactivateClient(c.id); this.closeModal(); }, 'danger') : null, UI.Btn('Delete', () => this.deleteClient(c.id, c.name), 'danger'), UI.Btn('Mark invoice paid', () => { this.markPaid(c.id); this.closeModal(); }, 'primary')], '640px');
    }
      ...
      try {
        ...
        if (pdfUrl || parts.length) {
          const contract = {
            ...
            notes: '', setup_fee: f.ccSetup ? +f.ccSetup : null, signing_link: '',
            signed_at: new Date().toISOString(),
