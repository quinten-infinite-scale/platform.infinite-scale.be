...
class Component extends DCLogic {
  ...
  async _poll() {
    ...
    try {
      ...
      // Real-time dials: refresh today's dials for all agents every poll cycle
      const todayDialsDate = new Date().toISOString().slice(0, 10);
      const rawDialsToday = await SB.get('dials', `?dial_date=eq.${todayDialsDate}&select=agent_id,count`).catch(() => null);
      if (rawDialsToday && rawDialsToday.length >= 0) {
        this.mutLocal(dd => {
          for (const row of rawDialsToday) {
            if (!dd.dials[row.agent_id]) dd.dials[row.agent_id] = {};
            dd.dials[row.agent_id][todayDialsDate] = row.count;
          }
        });
      }
  ...
  async saveApptEdits(id) {
    ...
    const fields = {
      ...
      date_appt: (() => { const d = f.eApptDate !== undefined ? f.eApptDate : (ap.dateAppt || '').slice(0, 10); const t = f.eApptTime !== undefined ? f.eApptTime : (ap.dateAppt?.includes('T') ? ap.dateAppt.slice(11, 16) : ''); const v = d + (t ? 'T' + t : ''); return v || null; })(),
      date_logged: (f.eApptDateLog !== undefined ? f.eApptDateLog : ap.dateLog) || null,
  ...
  async toggleTodo(id, me) {
    ...
    const todos = (agent.todos || []).map(t => {
      if (t.id !== id) return t;
      const isDone = !(t.done && t.doneDate === today);
      return isDone ? { ...t, done: true, doneDate: today } : { ...t, done: false, doneDate: null };
    });
  ...
  async saveClientEdits() {
    ...
    const closeFee = f.editCloseFee !== undefined ? +(String(f.editCloseFee).replace(/[^\d.]/g, '')) || null : (c.close_fee || null);
  ...
  renderVals() {
    ...
    // Me
    const roleLabels = { admin: 'Admin workspace', agency: 'Lead agency', client: 'Client portal', agent: 'Call agent', salesperson: 'Sales workspace', manager: 'Manager workspace' };
    ...
    // Nav
    const navDefs = {
      agent: [['dashboard', 'Dashboard', 'M3 3h7v7H3zM14 3h7v7h-7zM14 14h7v7h-7zM3 14h7v7H3z'], ['log', 'Appointment Log', 'M5 3h14v18H5zM9 9h6M9 13h6M9 17h4'], ['appointments', 'Appointments', 'M4 6h16M4 12h16M4 18h16'], ['eod', 'End of Day', 'M8 4h8M7 4h10v17H7zM10 10h4M10 14h4'], ['payments', 'Payments', 'M3 6h18v12H3zM3 10h18M7 15h3'], ['clients', 'My Clients', 'M9 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6M3 20a6 6 0 0 1 12 0M17 11a3 3 0 0 0 0-6M21 20a6 6 0 0 0-4-5.6'], ['rooster', 'Rooster', 'M8 2v4M16 2v4M3 10h18M5 4h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2zM10 16l2 2 4-4'], ['stats', 'Statistics', 'M4 20V10M10 20V4M16 20v-7M22 20H2'], ['tickets', 'Tickets', 'M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2M9 5a2 2 0 0 0 2 2h2a2 2 0 0 0 2-2M9 5a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2M12 12v4M10 14h4'], ['coaching', 'Coaching', 'M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75'], ['settings', 'Settings', 'M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6M12 2v3M12 19v3M2 12h3M19 12h3']],
      client: [['dashboard', 'Dashboard', 'M3 3h7v7H3zM14 3h7v7h-7zM14 14h7v7h-7zM3 14h7v7H3z'], ['appointments', 'Appointments', 'M4 6h16M4 12h16M4 18h16'], ['stats', 'Statistieken', 'M4 20V10M10 20V4M16 20v-7M22 20H2'], ['billing', 'Billing', 'M6 3h12v18l-3-2-3 2-3-2-3 2zM9 8h6M9 12h6'], ['legal', 'Legal', 'M12 3l8 3v6c0 5-4 8-8 9-4-1-8-4-8-9V6z'], ['support', 'Support', 'M4 5h16v11H9l-4 4z'], ['settings', 'Settings', 'M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6M12 2v3M12 19v3M2 12h3M19 12h3']],
      agency: [['dashboard', 'Dashboard', 'M3 3h7v7H3zM14 3h7v7h-7zM14 14h7v7h-7zM3 14h7v7H3z'], ['appointments', 'Appointments', 'M4 6h16M4 12h16M4 18h16'], ['stats', 'Statistieken', 'M4 20V10M10 20V4M16 20v-7M22 20H2'], ['billing', 'Billing', 'M6 3h12v18l-3-2-3 2-3-2-3 2zM9 8h6M9 12h6'], ['legal', 'Legal', 'M12 3l8 3v6c0 5-4 8-8 9-4-1-8-4-8-9V6z'], ['support', 'Support', 'M4 5h16v11H9l-4 4z'], ['settings', 'Settings', 'M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6M12 2v3M12 19v3M2 12h3M19 12h3']],
      admin: [['dashboard', 'Dashboard', 'M3 3h7v7H3zM14 3h7v7h-7zM14 14h7v7h-7zM3 14h7v7H3z'], ['finances', 'Finances', 'M3 7h18v12H3zM3 11h18M7 15h4'], ['stats', 'Statistics', 'M4 20V10M10 20V4M16 20v-7M22 20H2'], ['apptadmin', 'Appointments', 'M8 2v4M16 2v4M3 10h18M5 4h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z'], ['clients', 'Clients', 'M3 9l9-6 9 6v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2zM9 21V12h6v9'], ['agents', 'Call Agents', 'M9 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6M3 20a6 6 0 0 1 12 0M17 11a3 3 0 0 0 0-6M21 20a6 6 0 0 0-4-5.6'], ['salespeople', 'Salespeople', 'M16 7a4 4 0 1 1-8 0 4 4 0 0 1 8 0M12 14c-4.418 0-8 1.79-8 4v1h16v-1c0-2.21-3.582-4-8-4M20 8v6M17 11h6'], ['managers', 'Managers', 'M9 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6M3 20a6 6 0 0 1 12 0M17 11a3 3 0 0 0 0-6M21 20a6 6 0 0 0-4-5.6M12 3h9M12 7h9'], ['targets', 'Targets', 'M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z'], ['opa', 'OPA', 'M9 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6M3 20a6 6 0 0 1 12 0M22 8h-6M22 12h-5M22 16h-6'], ['clientsuccess', 'Client Success', 'M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75'], ['coaching', 'Coaching', 'M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75'], ['rooster', 'Roosters', 'M8 2v4M16 2v4M3 10h18M5 4h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2zM10 16l2 2 4-4'], ['eodadmin', 'EOD Reports', 'M8 4h8M7 4h10v17H7zM10 10h4M10 14h4'], ['timeline', 'Project Timeline', 'M4 5h16v15H4zM4 9h16M8 3v4M16 3v4'], ['prospects', 'Prospect CRM', 'M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18M12 7a5 5 0 1 0 0 10 5 5 0 0 0 0-10'], ['recruitment', 'Recruitment', 'M8 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6M2 20a6 6 0 0 1 12 0M18 8v6M15 11h6'], ['contracts', 'Contracts', 'M7 3h10v18H7zM10 8h4M10 12h4M10 16h2'], ['whatsapp', 'WhatsApp', 'M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z'], ['todos', 'To-Do', 'M9 11l3 3L22 4M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11'], ['tickets', 'Tickets', 'M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2M9 5a2 2 0 0 0 2 2h2a2 2 0 0 0 2-2M9 5a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2M12 12v4M10 14h4'], ['roadmap', '€100K Roadmap', 'M3 17l4-8 4 4 4-6 4 10M3 3h18v18H3z'], ['activity', 'Activity', 'M22 12h-4l-3 9L9 3l-3 9H2'], ['settings', 'Settings', 'M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6M12 2v3M12 19v3M2 12h3M19 12h3']],
      salesperson: [['prospects', 'Mijn Leads', 'M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18M12 7a5 5 0 1 0 0 10 5 5 0 0 0 0-10'], ['apptadmin', 'Afspraken', 'M8 2v4M16 2v4M3 10h18M5 4h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z'], ['todos', 'To-Do', 'M9 11l3 3L22 4M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11'], ['settings', 'Instellingen', 'M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6M12 2v3M12 19v3M2 12h3M19 12h3']],
    ...
    if (s.role === 'agent' && me) {
      ...
      out.workingTime = me.working && me.workSince ? ('since ' + new Date(me.workSince).toLocaleTimeString('nl-BE', { hour: '2-digit', minute: '2-digit' })) : '';
    ...
    // Page title
    const titles = { roadmap: '€100K Roadmap', dashboard: 'Dashboard', log: 'Appointment Log', appointments: 'Appointments', apptadmin: 'Appointments', eod: 'End of Day Report', payments: 'Payments', clients: s.role === 'admin' ? 'Clients' : 'My Clients', stats: 'Statistics', settings: 'Settings', billing: 'Billing', legal: 'Legal & Contracts', support: 'Support', finances: 'Finances', agents: 'Call Agents', eodadmin: 'End of Day Reports', timeline: 'Project Timeline', prospects: 'Prospect CRM', recruitment: 'Recruitment', contracts: 'Contracts', todos: 'To-Do', whatsapp: 'WhatsApp', targets: 'Targets', clientsuccess: 'Client Success', coaching: 'Coaching', salespeople: 'Salespeople', managers: 'Managers' };
  ...
  renderScreen(R, d, s) {
    ...
    if (s.role === 'salesperson') return ScreenAdmin.scrAdmin.call(this, d, s);
    if (s.role === 'manager') return ScreenAdmin.scrAdmin.call(this, d, s);
    return R('div', null, '');
  }
  ...
  _admWhatsApp(...a) { return ScreenAdmin._admWhatsApp.call(this, ...a); }
  _admSalespeople(...a) { return ScreenAdmin._admSalespeople.call(this, ...a); }
  _admManagers(...a) { return ScreenAdmin._admManagers.call(this, ...a); }
